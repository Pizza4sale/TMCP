    ...
    if(isset($_SESSION['shopping_cart'])){
        ...
        $product_ids = array_column($_SESSION['shopping_cart'], 'id');
            ...
            if(!in_array($_GET['product_id'], $product_ids)){//** */
                ...
                //INSERT CART ITEM DETAILS TO TABLE cartitem
                $Q_insert_into_cartitem = 'INSERT INTO cartitem (productID, cartID, price, quantity,message_c) 
                VALUES ('.$_SESSION['productID'].','.$_SESSION['cartID'].','.filter_input(INPUT_POST, 'price').','.filter_input(INPUT_POST, 'input_quantity').',"'.$_POST['pg1'].'" )';
                $run_insert_into_cartitem = mysqli_query($conn, $Q_insert_into_cartitem);
                    }
            }
            }
                ...
                for($i=0; $i<count($product_ids); $i++){
                    ...
                    //filter_input(INPUT_GET, 'product_id')){
                        ...
                        {
                              //UPDATE QUERY IN TABLE cartitem
                        $Q_update_cartitem = 'UPDATE cartitem SET quantity = '.$_SESSION['shopping_cart'][$i]['quantity'].' ,message_c ="'.$_POST['pg1'].'"
                        WHERE productID = '.$_GET['product_id'];
                        $run_update_cartitem = mysqli_query($conn, $Q_update_cartitem);
                        }
    ...
    <body>
        ...
        <?php
             if(isset($_GET['product_id'])){ //if(isset($_GET['product_id'])){
                ...
                $categoryID = isset($row_cat_id['categoryID']) ? $row_cat_id['categoryID'] : null;
                    ...
                    <div class="col mt-4">
                        ...
                        <form id="form-pd" method="POST" action="product.php?action=add&product_id=<?php echo $product_id; ?>">
                            <div class="box my-4">
                                ...
                                <input type="hidden" class="cartID" name="cartID" value="<?php echo isset($_SESSION['cartID']) ? $_SESSION['cartID'] : ''; ?>" />

                                <br>
                                <input type="hidden" class="show_id" name="productID_id" value="<?php echo $product_id;?>" />
                                <?php if($categoryID==1){ ?>
                                <br>
                                 <input class="input-mx-2 p-3 px-4" name="pg1" required type="text" placeholder="Personalized Greeting"/>
                                 <br>
                                 <span>1 Free Candle</span>
                                 <br> Note: You need to go to our store to pay downpayment to start your customize cake
                                 <br>
                             <?php }?>
